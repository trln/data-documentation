...
=== UNCb9058990
_Example shows special 502 field processing_

_590 processing will be institution-specific_

 502 _ _ $aThesis (Ph. D.)--University of Lancaster, Lancashire, England.
 506 1 _ $aAccess limited to UNC Chapel Hill-authenticated users.$fUnlimited simultaneous users
 590 _ _ $aProvider: SpringerLink.
...
----
{
  ...
  "note_access_restriction":[
    {
      "value":"Access limited to UNC Chapel Hill-authenticated users. Unlimited simultaneous users"
    }
  ]
}
----
...
_588 is NOT mapped to Argot._

 502 _ _ $gRecital document$bMaster of Music in Performance and Vocal Pedagogy$cUniversity of Texas at San Antonio$d2012.
 504 _ _ $aIncludes bibliographical references (pages 58-64).
 538 _ _ $aSystem requirements: Adobe Acrobat reader.
 588 _ _ $aDescription based on online resource; title from PDF title (viewed April 15, 2013).
...
----
{
  "note_display":[
    ...
    {
      "value":"Includes bibliographical references (pages 58-64)."
    },
    {
      "value":"System requirements: PDF reader."
    }
  ],
  "note_index":[
...
----
{
  ...
  "note_index":[
    "Bremen International Graduate School of Social Sciences, 2008"
  ]
}
----
...
===== If first subfield is a and there are no subfields b,c,d,g, or o

* take contents of $a

_See UNCb9058990 example above._

===== If first subfield is a, followed by subfields b,c,d,g, or o

* Build note field from all subfield elements present, following punctuation provision rules

_This example is using mock data, since the records like this in the UNC catalog were already cleaned up._
...
See UNCb7386333 and UNCb1511334 examples above.
...
See UNCb6856070 example above.

[NOTE]
====
Unless it's easier to use the constructed display value in this case, it would be fine to just use the actual field data in `note_index`.

See UNCb6856070 example above.
====

==== Punctuation rules for building field value from component subfields (non-$a-only field values
...
[NOTE]
====
If I recall correctly, punctuation is stripped out of or ignored in the indexed fields.

If that's true, then if it is easier or better to NOT stick the punctuation in the values sent to `note_indexed`, that seems fine.
====
